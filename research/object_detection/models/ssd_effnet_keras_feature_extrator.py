...
"""SSDFeatureExtractor for Effnet features."""
...
class SSDEffNetKerasFeatureExtractor(
    ssd_meta_arch.SSDKerasFeatureExtractor):
  """SSD Feature Extractor using Effnet features."""
  ...
  def __init__(self,
               ...
               name=None):
    """Effnet Feature Extractor for SSD Models.

    Effnet v2 (experimental), designed by andreatramo@.

    Args:
      is_training: whether the network is in training mode.
      depth_multiplier: float depth multiplier for feature extractor (Functions
        as a width multiplier for the effnet network itself).
    ...
    """
    super(SSDEffNetKerasFeatureExtractor, self).__init__(
  ...
  def build(self, input_shape):
      full_effnet = effnet.effnet(
        ...
        include_top=False)
    conv2d_11_pointwise = full_effnet.get_layer(
        name='block_13_expand_relu').output
    conv2d_13_pointwise = full_effnet.get_layer(name='out_relu').output
    self.mobilenet_v2 = tf.keras.Model(
        inputs=full_effnet.inputs,
