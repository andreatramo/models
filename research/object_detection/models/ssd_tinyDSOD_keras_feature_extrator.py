...
"""SSDFeatureExtractor for Tiny-DSOD features."""
...
class SSDTinyDSODKerasFeatureExtractor(
    ssd_meta_arch.SSDKerasFeatureExtractor):
  """SSD Feature Extractor using Tiny-DSOD features."""
  ...
  def __init__(self,
               ...
               name=None):
    """Tiny-DSOD Feature Extractor for SSD Models.

    Tiny-DSOD (experimental), designed by andreatramo@.

    Args:
      is_training: whether the network is in training mode.
      depth_multiplier: float depth multiplier for feature extractor (Functions
        as a width multiplier for the Tiny-DSOD network itself).
    ...
    """
    super(SSDTinyDSODKerasFeatureExtractor, self).__init__(
  ...
  def build(self, input_shape):
      full_tinyDSOD = tinyDSOD.tinyDSOD(
        ...
        include_top=False)
    conv2d_11_pointwise = full_tinyDSOD.get_layer(
        name='block_13_expand_relu').output
    conv2d_13_pointwise = full_tinyDSOD.get_layer(name='out_relu').output
    self.mobilenet_v2 = tf.keras.Model(
        inputs=full_tinyDSOD.inputs,
